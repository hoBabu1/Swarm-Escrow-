...
function VerdictCard({
  ...
  if (!hasVoted) {
    if (notNeededReason) {
      return (
        <div style={{ background: 'rgba(6,10,12,0.5)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 22, minWidth: 0, overflowWrap: 'anywhere' }}>
  ...
  return (
    <div style={{ background: 'rgba(6,10,12,0.5)', border: `1px solid ${approved ? 'rgba(77,255,184,0.25)' : 'rgba(255,180,77,0.25)'}`, borderRadius: 16, padding: 22, minWidth: 0, overflowWrap: 'anywhere' }}>
  ...
  const handleSubmitDeliverable = () => {
    if (!canSubmitDeliverable || escrowId === undefined) return;
          ...
          @media (min-width: 640px) {
            .agent-verdict-grid {
              grid-template-columns: repeat(var(--verdict-card-count), minmax(0, 1fr));
            }
          }
        `}</style>
              ...
              {escrowData.status !== EscrowStatus.Created && (() => {
                ...
                return (
                  <>
                    ...
                    <div className="agent-verdict-grid" style={{ '--verdict-card-count': cardCount } as React.CSSProperties}>
                      {verdicts.map((v, i) => (
                        <VerdictCard
                          key={v.agent}
                          agent={v.agent}
